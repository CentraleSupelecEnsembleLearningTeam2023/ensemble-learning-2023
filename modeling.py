...
def train_linear_reg(X_train,y_train,X_test,y_test, cross_val = False):
  '''
  Function to conduct linear regression model.
  with option for cross validation
  '''
...
def train_decision_tree(X_train,y_train,X_test,y_test,max_depth= None,min_samples_split = 2,cross_val = False,grid_search = False):
  '''
  :param max_depth: set max_depth of decision tree
  :param min_samples_split: set minimal samples per split
  :param cross_val: boolean, provide option for cross validation
  :param grid_search: boolean, provide option for grid search
...
def train_random_forest(X_train,y_train,X_test,y_test,estimators = 100,max_depth= None,min_samples_split = 2,cross_val = False,grid_search = False):
  '''
  :param estimators: set number of estimators
  :param max_depth: set max_depth
  :param min_samples_split: set minimal samples of the split
  :param cross_val: boolean, option for cross validation
  :param grid_search: boolean, option for grid search
